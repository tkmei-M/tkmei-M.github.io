...
function getChapterCharaIdFromURL() {
  const params = new URLSearchParams(window.location.search);
  const charaId = params.get("chapterCharaId");
  return charaId ? String(charaId) : null;
}
...
function getChapterTimeRange(chapterCharaId, nowTimestamp) {
  const currentEventTimeRange = findCurrentEventTimeRange(
    eventsCache,
    nowTimestamp,
  );

  if (!chapterCharaId || !currentEventTimeRange?.eventId) {
    return currentEventTimeRange;
  }

  const targetChapter = (chaptersCache || []).find((chapter) => {
    return (
      Number(chapter?.eventId) === Number(currentEventTimeRange.eventId) &&
      Number(chapter?.gameCharacterId) === Number(chapterCharaId) &&
  ...
  const chapterStartAt = Number(targetChapter?.chapterStartAt);
  const aggregateAt = Number(targetChapter?.aggregateAt);

  return {
    eventId: Number(currentEventTimeRange.eventId),
    startTimestamp: Number.isFinite(chapterStartAt)
      ? getMinuteTimestamp(chapterStartAt)
      : currentEventTimeRange.startTimestamp,
    endTimestamp: Number.isFinite(aggregateAt)
      ? getMinuteTimestamp(aggregateAt)
...
async function fetchAndDisplayUserData() {
  ...
  try {
    ...
    const charaId = getChapterCharaIdFromURL();
    ...
    const { data, error } = await client
      ...
      .match({ userId: String(userId), gameCharacterId: String(charaId) })
    ...
    const rankConditions = [String(currentRank), String(anotherPlotRankValue)];

    let rankOneStartTimestamp = null;
    const { data: rankOneData, error: rankOneError } = await client
      .from("Chapter_eventBoard_archive")
      .select("TimeStamp")
      .match({ rank: 1, gameCharacterId: String(charaId) })
      .order("TimeStamp", { ascending: true })
      .limit(1);
    ...
    const { data: rankData, error: rankError } = await client
      ...
      .eq("gameCharacterId", String(charaId))
      .order("TimeStamp", { ascending: true });
    ...
    const currentEventTimeRange = getChapterTimeRange(charaId, nowTimestamp);
...
function createScoreDifferenceTable(
  ...
  // 0行目のスコアがnullの場合、0に置換
  if (scores.length > 0 && scores[0] === null) {
    scores[0] = 0;
  }
  if (rankSeriesData.length > 0) {
    rankSeriesData.forEach(({ values }, seriesIndex) => {
      if (values.length > 0 && values[0] === null) {
        rankSeriesData[seriesIndex].values[0] = 0;
      }
    });
  }
  let previousScore = null;
  ...
  rankSeriesData.forEach(({ rank }, seriesIndex) => {
...
function createScoreDifferenceHourlyTable(
  ...
  // ランクのスコアも同様に、0個目のスコアがnullの場合、0に置換
  if (rankSeriesData.length > 0) {
    rankSeriesData.forEach((series) => {
      if (series.values.length > 0 && series.values[0] === null) {
        series.values[0] = 0;
      }
    });
  }
